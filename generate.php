...
<body>
 ...
 <nav class="bg-white shadow-lg fixed w-full z-10 top-0">
  ...
  <a href="generate.html" style="color:blue;margin:auto;">Add Another</a><br>
...
if(!$conn){
    ...
    $sql="Insert into certificate(name,event,position,date,venue,dean_name,dept,email) values('$name', '$event','$position','$date','$venue','$dean','$dept','$email')";
...
$sql = "SELECT serial_number FROM certificate WHERE name = '$name' AND date = '$date' AND venue = '$venue' AND event = '$event' AND position = '$position' ";
    $result=mysqli_query($conn, $sql);
    if($result){
...
try {
    ...
    You have earned a certificate for $position in $event on $date.<br>Kindly find the below the Serial Number and enter this on Certify'em.com to downlo
